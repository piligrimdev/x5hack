...
- `POST /basket/checkout` не поддерживает `points_to_spend` — в отличие от терминального `/receipts`
- Заказ из AI-корзины всегда оплачивается без апельсинок, даже если у пользователя есть баланс
- Нужно прокинуть `points_to_spend` через `CheckoutRequest` в `ReceiptService.create_receipt`
