# Backlog

## В PoC не делаем (откладываем)

### Аватар-механика
- Таблица прогресса аватара (уровень, очки, история изменений)
- Кастомизация аватара за баллы
- Печать аватара на чеке (офлайн/онлайн)
- Связь: карта лояльности → аватар

### Районный рейтинг экономии
- Сущность "магазин" с геокластером/районом
- Агрегированная таблица/view рейтинга по кластерам
- Анонимное сравнение: юзер vs. дом/район (без ФИО/адресов)
- Периодические снэпшоты рейтинга

### Купоны
- Схема сущности купона и связей

### Фрод
- Скоринг по uid карт оплаты (сигнал: 4 последние банковские карты на карту лояльности)

### Апельсинки / баллы лояльности
- ~~Таблица транзакций баллов: начисление (за задание), списание (на кассе)~~ — реализовано в feature 007 (`points_transaction`).
- ~~Кэш-баланс на карте лояльности (денормализованный)~~ — реализовано (`points_account.balance`).
- ~~Интеграция апельсинок как частичной оплаты чека (points_redeemed на чеке)~~ — реализовано (`receipt.cashback_applied_*`).
- **Не реализовано** (осталось):
  - Начисление баллов за сам факт покупки (сейчас — только за задания).
  - Срок истечения баллов (`expired_at`, автосгорание).
  - Возврат/отмена чека — откат `spend`, отзыв `earn` за отменённое задание.
  - Cleanup orphan `points_account` при удалении `loyalty_card`.
  - Push-уведомление о начислении баллов.
  - Дробный курс (Decimal `rate_points_per_rub`) — сейчас только integer.

### Cashback (feature 007) — переосмысление устаревших полей
- `task.reward_id` и `task.reward_type` (введены в feature 006) новыми задачами не заполняются: награда = баллы, а не Discount. Отдельной миграцией удалить/переосмыслить.
- `TaskRepository.create_reward_discount` помечен `# BACKLOG-cleanup` — удалить после подтверждения, что нет внешних вызовов.
- Sanity-check скрипт: пересчитывать `points_account.balance = SUM(amount) FROM points_transaction WHERE account_id=...` периодически, для выявления рассинхронизации.

### Concurrent-spend integration test (SC-003)
- Реальный integration-тест с 100 параллельными потоками против живой Postgres, проверяющий инвариант `balance >= 0` (сейчас — только логика через MagicMock + DB constraint).

### Составные задания (multi-criterion)
- task_progress таблица: task_id, criterion_id, current_value, target_value
- Поддержка нескольких критериев на одно задание

### Скидки по расписанию
- Скидки по дням недели / времени суток (is_recurrent, schedule)

### Уценка как фиксированная сумма
- ~~value_type на скидке (percent / fixed_rub)~~ — реализовано в feature 006 для награды за задание. Уценку кассой ещё нужно смоделировать явно.

### Персистентная корзина ассистента — частично реализовано
- На устройстве корзина теперь сохраняется (`@react-native-async-storage/async-storage`,
  ключ `@x5hack/weeklyBasket`) и переживает переходы между экранами — правки через
  ассистента "Аппи" и результат «Собрать корзину на неделю» больше не теряются
- **Не сделано:** персистентность НЕ привязана к пользователю — один ключ на устройство,
  общий для всех аккаунтов. При смене пользователя на одном устройстве (без переустановки
  приложения) корзина от предыдущего аккаунта подхватится новым — пересекаются только
  `product_id`/`quantity` уже выбранных товаров каталога, без имён/цен/истории покупок
- Для продакшна нужна серверная персистентность на пользователя (таблица вида
  `user_baskets`) или как минимум ключ AsyncStorage, включающий user_id

### Дополнительные типы наград (мост FR-011a готов)
- Coupon-сущность (id, code, discount_value, expires_at, used_at) как второй `task.reward_type`
- Points/апельсинки как третий `task.reward_type` (требует таблицы транзакций баллов из «Апельсинки»-раздела)

### Langfuse интеграция
- Экспортировать записи `challenge_generation_log` как traces + tool-calls в Langfuse для внешнего аудита LLM
- Сейчас — только Postgres-таблица

### Unit-тесты Celery-задач с live Postgres
- test_generation_and_receipt.py, test_process_receipt_progress.py, test_expiration.py — требуют pytest-фикстуры на реальную БД
- Логика частично покрыта unit-тестами task_completion/challenge_service

### Adapter unit-tests
- `web/tests/webx5/services/test_challenge_adapter.py` — детально проверить `build_profile` (margin из config), `_lookup_product` (ILIKE + ordering), `persist_challenge` для всех известных `SCRIPT_FIELD_TO_CRITERION_KIND`

### Defensive null-check в BasketService.checkout()
- `store_repo.get_by_id(session, receipts[0].store_id)` используется без проверки на `None`
- Сейчас недостижимо: `Receipt.store_id` — `ForeignKey("stores.id", ondelete="RESTRICT")`, БД не даст удалить
  магазин, на который ссылается существующий чек
- Стоит добавить `if store is None: raise HTTPException(422, ...)` как страховку на случай, если политика FK
  изменится (например, на `SET NULL`/`CASCADE`)

### ~~Списание баллов лояльности при оформлении заказа из корзины~~
- Реализовано: `POST /basket/checkout` принимает `points_to_spend` (только `"all"` через тумблер
  «Списать баллы» в UI, точное количество не поддерживается — см. новую запись ниже)

### Выбор магазина пользователем вручную при оформлении заказа
- Сейчас магазин для `/basket/checkout` выбирается автоматически: последний магазин из истории
  чеков пользователя, либо первый магазин в БД, если истории нет
- Пользователь не может указать другой магазин (например, ближайший или тот, где планирует быть)
- Нужен явный `store_id` (опциональный) в `CheckoutRequest` с фолбэком на текущую эвристику

### Рефактор terminal-эндпоинта create_receipt на build_receipt_response()
- `web/src/webx5/routes/receipts.py::create_receipt` (строки ~140-184) собирает `ReceiptResponse`
  вручную, дублируя логику, которую `ReceiptService.build_receipt_response()` уже инкапсулирует
  для нового `/basket/checkout`
- Свести оба места к одному вызову `receipt_service.build_receipt_response()`

### Хардненинг checkout-эндпоинта (basket)
- Нет идемпотентности у `POST /basket/checkout` — `receipt_id` генерируется как `uuid4()` на
  каждый вызов, в отличие от терминального `/receipts`, который требует `X-Idempotency-Key`;
  таймаут на мобильной сети может привести к повторному чеку при ретрае пользователя
- `BasketItemIn.quantity` / `CheckoutRequest.items` не имеют верхней границы —
  `quantity=999999999` технически проходит валидацию

### Человекочитаемые ошибки на фронтенде корзины
- `apiFetch` бросает `Error("422 {...}")`, и `useBasket.ts` показывает это как есть через
  `setMessage(e.message)` — пользователь видит код статуса и сырой JSON вместо текста
- Стоит парсить `detail` из тела ошибки перед показом пользователю

### Точный ввод количества баллов для списания
- Тумблер «Списать баллы» в корзине сейчас — только вкл/выкл, списывает максимум
  (`points_to_spend: "all"`)
- Точный ввод числа баллов (как у терминального `/receipts/calculate`, которое принимает
  произвольный `int`) — вне объёма, добавить как отдельное поле ввода при необходимости

### Lazy import basket_service в роутах (tech debt)

- `routes/basket.py` импортирует `basket_service` внутри каждого хендлера (`from webx5.core.basket import basket_service`) вместо уровня модуля — нетипично для остальных роутов
- Сделано для обхода circular import через цепочку `server.py → routes/basket.py → core/basket.py`; работает корректно, но нарушает единообразие
- Долгосрочно: перенести wiring `basket_service` в `core/server.py` (или `lifespan`) и передавать через `request.state` / dependency

### Unit-экономика LLM-вызовов ассистента корзины

- Нет расчёта стоимости одного вызова `POST /basket/assistant` (Принцип IV конституции)
- claude-haiku-4.5 через OpenRouter: ~$0.001 за вызов при среднем размере каталога
- Для демо: указать примерную стоимость на 1 000 активных пользователей/день как часть pitch-слайда unit-экономики

### Синтетические телефоны: исчерпание пространства при большом сиде

- `seed_receipts.py` генерирует телефоны в диапазоне `+7900xxxxxxx` (10M номеров)
- При засеве > ~3M уникальных пользователей вероятность коллизии существенно растёт; retry до 20 попыток страхует конкретную коллизию, но не исчерпание
- Для PoC с синтетикой ~10K–100K пользователей — приемлемо; для большого сида добавить несколько префиксов (+7901, +7902 и т.д.)

### `apply_discount` как публичная функция модуля

- Переименована из `_apply_discount` ради переиспользования в `receipt.py`; нарушает инкапсуляцию
- Правильное решение: метод на `DiscountCalculatorService` (или статический метод), который вызывают оба места
- Сейчас функция — чистая математика (без состояния), поэтому риск минимален, но контракт модуля неявно расширен

### Общий хелпер ценообразования корзины (preview + terminal)
- `BasketService.preview()` (`web/src/webx5/services/basket_assistant.py`) — почти дословная копия
  `routes/receipts.py::calculate_discounts` (~45 строк): тот же маппинг в `CalculatedItemOut`,
  та же сборка `total_base`/`total_paid`/`total_saved`/`CashbackBlock`
- Стоит вынести в общий сервисный метод, который вызывают оба места — заодно уберёт SQL
  (`session.get(Store, ...)`, `select(Product.id)`), который сейчас лежит прямо в
  `calculate_discounts`, нарушая RSI (роут не должен делать SQL напрямую)
- Не сделано в рамках текущего плана — узкий скоуп, оставлено как есть

## Персональные челленджи: receptiveness/saturation-гейтинг живой выдачи

`compute_receptiveness` и `compute_frequency_saturation` (`synth/challenges.py`)
больше не определяют, какой микс челленджей получит живой пользователь —
с 2026-09-05 каждый пользователь получает одинаковый 4-слотовый микс
(`llm_habit`, `llm_discovery`, `generic`, `vibe`) независимо от силы
покупательского паттерна или частоты покупок. Обе функции, а также
`build_spend_threshold_challenge`/`build_category_expansion_challenge`,
остаются в коде и используются `synth/simulation.py` для офлайн-симуляции
экономического эффекта — удалять их нельзя. Решение отказаться от
гейтинга в живой выдаче принято сознательно (см.
`docs/superpowers/specs/2026-09-05-challenge-mix-vibe-design.md`), не баг.

## Персональные челленджи: находки финального ревью 4-слотового микса (2026-09-05)

Найдено при финальном whole-branch ревью ветки `challenge-mix-vibe` — не
исправлено кодом сознательно, требует отдельного решения/действия:

- **Тема "Забота о питомце" перегружена по охвату.** `VIBE_CATEGORIES`
  (`synth/challenges.py`) делит 20 категорий на 6 тем поровну по числу
  пользователей (`pick_vibe_category` — равномерный хеш), но тема "Забота о
  питомце" содержит ровно 1 категорию (`товары для животных`), а
  `allowed_categories` в `parse_and_validate_challenge` жёстко запрещает
  выход за пределы темы. Эмпирически измерено на 6000 синтетических
  пользователях: ~16.7% (1 из 6) каждый месяц получают vibe-слот, жёстко
  привязанный к товарам для животных, независимо от того, есть ли у них
  питомец — сигнала о владении питомцем в профиле нет вообще. Это бьёт по
  релевантности (H2/Принцип III). Варианты решения — взвесить
  `pick_vibe_category` по размеру темы, влить тему в более широкую, или
  обусловить назначение темы наличием покупок в её категориях — не
  выбраны, требуют продуктового решения, не технической правки.

- **Стоимость/латентность LLM-генерации выросла в 3 раза не перепроверена
  против SC-001.** Раньше на пользователя был 0-1 вызов LLM (только слот
  `llm`, детерминированные `spend_threshold`/`category_expansion` без
  сети); теперь до 3 последовательных вызовов (`llm_habit`,
  `llm_discovery`, `vibe`). `specs/006-user-challenges/plan.md`/SC-001
  закладывают ~30 сек на появление заданий после первого чека — это
  проектная цель предыдущей итерации, актуальность после утроения вызовов
  не подтверждена эмпирически (нет доступа к реальным замерам латентности
  в рамках этого ревью). Требуется замер на реальном LLM-эндпоинте перед
  тем, как полагаться на эту цифру.

- **`specs/006-user-challenges/spec.md` описывает предыдущую итерацию и
  противоречит текущему коду.** FR-001 ("не более 3 активных заданий"),
  FR-005/FR-005a (микс из трёх типов: llm + spend_threshold +
  category_expansion), FR-015 ("максимум 3 задания в ответе") — всё это
  зафиксировано под старый 3-слотовый дизайн. Код (`challenge.py`) при
  этом ссылается на **тот же FR-001** для инварианта "не более 4". Спека
  не переписана (это замороженный документ прошлой итерации), но
  расхождение стоит иметь в виду при следующей ревизии `specs/006-*`.

## Персональные челленджи: 5-й слот llm_basket (2026-09-05)

`CHALLENGE_SLOTS` вырос до 5 (`llm_habit`, `llm_discovery`, `llm_basket`,
`generic`, `vibe`) — добавлен `llm_basket`, оборачивающий существующую фичу
"предложенная корзина на неделю" (`BasketRepository.suggest_items`) в LLM-
челлендж. Инвариант "макс. активных заданий" вырос до 5 автоматически (код
уже вычислял его как `len(CHALLENGE_SLOTS)`, без хардкода).

Мобильный экран заданий рассчитан на переменное число карточек (список,
без хардкода на 3/4) — но 5 карточек одновременно ещё не проверялись вживую;
стоит визуально проверить экран при первой возможности, как и было
рекомендовано при переходе 3→4.

Добавлен дедуп между последовательными циклами генерации: если резолвленный
критерий слота совпадает с тем, что этот же слот выдавал в прошлый раз
(`TaskRepository.get_last_criterion_per_slot`), слот пропускается в этом
цикле — не пытается перегенерироваться с другим seed, просто ждёт
следующего триггера. См. `docs/superpowers/specs/2026-09-05-challenge-basket-slot-dedup-design.md`.

### Известное ограничение: слот `generic` фактически одноразовый на пользователя

Найдено при финальном whole-branch ревью — **требует решения, не исправлено
кодом**. Единый дедуп между циклами (см. выше) сравнивает резолвленный
критерий слота с историей ТОГО ЖЕ слота. Для `generic` это ломается
структурно: его выбор (`_pick_distinct_generic_offer`'s хеш) — чистая
функция `user_id` без временной/цикловой компоненты, поэтому результат
идентичен на каждом цикле. Начиная со второго цикла генерации `previous_by_slot["generic"]`
всегда совпадает с новым резолвленным критерием — слот пропускается
**навсегда**, а не время от времени. Слабее, но по той же причине,
страдает и cold-start-фолбэк `llm_basket` (детерминирован при стабильно
пустой истории), `vibe` защищён только частично (тема меняется раз в
месяц через `pick_vibe_category`, но внутри месяца тоже статична).

Спека сознательно отвергла альтернативу "предзаполненный индекс в
`_pick_distinct_generic_offer`" как избыточно сложную для той же цели — но
именно этот механизм позволил бы `generic` вращаться между офферами.
Это ограничение дизайна спеки, не баг реализации: код построен ровно так,
как спека просила.

**Нужно решение** (не выбрано): принять `generic` как one-shot-навсегда
слот (тогда после первого цикла у пользователя фактически 4 живых слота
из 5), или добавить ротацию (например, сид `_pick_distinct_generic_offer`
чем-то меняющимся между циклами — счётчик прошлых `generic`-заданий
пользователя, доступный через `Task`-историю). См. полный разбор в отчёте
финального ревью этой ветки.
