...
class BasketRepository:
    def suggest_items(
        ...
        min_date, max_date = session.execute(
            select(
                func.min(Receipt.purchase_date), func.max(Receipt.purchase_date)
    ...
    def get_shopping_context(self, session: Session, user_id: uuid.UUID) -> dict:
        ...
        first = session.scalar(
            select(func.min(Receipt.purchase_date)).where(
                Receipt.loyalty_card_id == user_id,
                Receipt.purchase_date >= since,
            )
        )
        weeks = (
            max((now.replace(tzinfo=None) - first.replace(tzinfo=None)).days / 7, 1)
            if first
            else 1
        )
        rows = session.execute(
        ...
        tasks = session.scalars(
            ...
            .join(TaskStatus)
            .where(
                Task.loyalty_card_id == user_id,
                TaskStatus.name == "открыто",
                Task.deadline > now,
            )
        ...
        ctx: dict = {
            ...
            "typical_weekly_spend_rub": round(
                sum(float(row[4]) for row in rows) / weeks, 2
            ),
            "purchases": [
                {
                    "sku_id": sku,
                    "weekly_quantity": round(float(qty) / weeks, 2),
            ...
            "challenges": [
                self.challenge_context(t, catalog, forbidden) for t in tasks
            ],
        }
        user = session.get(User, user_id)
        if user is not None and user.vibe_type is not None:
            ctx["vibe_context"] = user.vibe_type.llm_context
        return ctx

    @staticmethod
    def challenge_context(
        task: Task, catalog: list[Product], forbidden: set[str]
    ) -> dict:
        # Match the same product/category and exclusion rules as task completion.
        matching = [
            p
            for p in catalog
            if (p.category.name if p.category else "") not in forbidden
            and (
                (task.criterion_type == "product" and p.id == task.criterion_entity_id)
                or (
                    task.criterion_type == "category"
                    and p.category_id == task.criterion_entity_id
                )
            )
        ]
        criteria = [
        ...
        return {
            ...
            "supported": bool(criteria)
            and task.criterion_type in {"product", "category"}
            and all(
                c.kind in {"item_quantity", "spend_threshold_rub"}
                for c in task.criteria
            ),
        }
