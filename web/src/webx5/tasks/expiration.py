...
def expire_tasks() -> dict:
    ...
    # One combined call per user, regardless of how many of their slots
    # expired — generate_batch fills every currently-missing slot in one
    # shot (see its docstring), so there's nothing to gain from a separate
    # call per expired task.
